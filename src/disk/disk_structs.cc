...
static_assert(sizeof(Daemons::Daemon) ==
                  sizeof(Daemons::Daemon::type) + sizeof(Daemons::Daemon::func),
              "Daemons::Daemon size has changed");
template <>
void Disk::save<Daemons::Daemon>(tag_type tag, Daemons::Daemon const& element,
                                 std::ostream& data) {
  save_tag(tag, data);

  data.write(reinterpret_cast<char const*>(&element.type),
             sizeof(element.type));
  data.write(reinterpret_cast<char const*>(&element.func),
             sizeof(element.func));
}
template <>
...
static_assert(sizeof(Daemons::Fuse) ==
                  sizeof(Daemons::Fuse::type) + sizeof(Daemons::Fuse::func) +
                      sizeof(Daemons::Fuse::func),
              "Daemons::Func size has changed");
template <>
void Disk::save<Daemons::Fuse>(tag_type tag, Daemons::Fuse const& element,
                               std::ostream& data) {
  save_tag(tag, data);
  ...
  data.write(reinterpret_cast<char const*>(&element.func),
             sizeof(element.func));
  data.write(reinterpret_cast<char const*>(&element.time),
             sizeof(element.time));
}
template <>
...
static_assert(sizeof(Coordinate) ==
                  sizeof(Coordinate::x) + sizeof(Coordinate::y),
              "Coordinate size has changed");
template <>
void Disk::save<Coordinate>(tag_type tag, Coordinate const& element,
                            std::ostream& data) {
  save_tag(tag, data);
...
// damage
static_assert(sizeof(damage) == sizeof(damage::sides) + sizeof(damage::dices),
              "damage size has changed");
template <>
void Disk::save<damage>(tag_type tag, damage const& element,
                        std::ostream& data) {
  save_tag(tag, data);
  data.write(reinterpret_cast<char const*>(&element.sides),
             sizeof(element.sides));
  data.write(reinterpret_cast<char const*>(&element.dices),
             sizeof(element.dices));
}
template <>
