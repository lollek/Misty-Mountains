...
static attack_modifier calculate_attacker(Character const& attacker,
                                          Item* weapon, bool thrown) {
  attack_modifier mod;

  // Weapons override base attack (at the moment, monsters shouldn't use
  // weapons)
  if (weapon != nullptr) {
    ...
    if (thrown) {
      ...
      class Weapon const* bow{player->equipped_weapon()};
      class Weapon const* w_weapon{dynamic_cast<class Weapon*>(weapon)};
    ...
    // But otherwise we use the attacker's stats (can happen to both monsters
    // and player)
  } else {
...
static bool roll_attacks(Character* attacker, Character* defender, Item* weapon,
                         bool thrown) {
  if (attacker == nullptr) {
  ...
  if ((defender == player && player_turns_without_action) ||
      defender->is_held() || defender->is_stuck()) {
    mod.to_hit += 4;
  }
  ...
  for (damage const& dmg : mod.damage) {
    ...
    if (fight_swing_hits(attacker->get_level(), defense, mod.to_hit)) {
      int const damage = roll(dmg.dices, dmg.sides) + mod.to_dmg;
...
static void print_attack(bool hit, Character* attacker, Character* defender) {
  stringstream ss;
  ss << attacker->get_name() << " " << attacker->get_attack_string(hit) << " "
     << defender->get_name();
...
int fight_swing_hits(int at_lvl, int op_arm, int wplus) {
  int const rand{os_rand_range(20) + 1};
...
void fight_missile_miss(Item const* weap, string const& mname,
                        string const* name_override) {
  if (weap->o_type == IO::Weapon) {
    if (name_override == nullptr) {
      Game::io->message("the " +
                        Weapon::name(static_cast<Weapon::Type>(weap->o_which)) +
                        " misses " + mname);
    } else {
      Game::io->message("the " + *name_override + " misses " + mname);
    }
  } else {
