...
bool Armor::is_magic() const {
  return (rustproof || get_armor() > Armor::ac(subtype));
}
...
static Armor::Type random_armor_type() {
  ...
  switch (Game::current_level) {
    ...
    case 2:
      ...
      [[clang::fallthrough]];
    case 1: potential_armor.push_back(Armor::Robe);
  }

  return potential_armor.at(os_rand_range(potential_armor.size()));
}

Armor::Armor(bool random_stats) : Armor(random_armor_type(), random_stats) {}

Armor::Armor(std::istream& data) { load(data); }
...
void Armor::set_identified() { identified = true; }

bool Armor::is_identified() const { return identified; }
...
string Armor::get_description() const {
  ...
  int const bonus_ac{get_armor() - ac(subtype)};

  buffer << "a" << vowelstr(obj_name) << " " << obj_name << " [" << ac(subtype);

  if (identified) {
    buffer << ",";
    if (bonus_ac > 0) { buffer << "+"; }
    buffer << bonus_ac;
  }
...
void Armor::set_rustproof() { rustproof = true; }

bool Armor::is_rustproof() const { return rustproof; }

int Armor::get_base_value() const { return value(subtype); }
...
bool Armor::is_stackable() const { return false; }

bool Armor::autopickup() const { return option_autopickup(o_type); }
