...
void Item::set_x(int new_value) { position_on_screen.x = new_value; }

void Item::set_y(int new_value) { position_on_screen.y = new_value; }

void Item::set_nickname(std::string const& new_value) { nickname = new_value; }

Coordinate const& Item::get_position() const { return position_on_screen; }

int Item::get_x() const { return position_on_screen.x; }

int Item::get_y() const { return position_on_screen.y; }

string const& Item::get_nickname() const { return nickname; }

int Item::get_item_type() const { return o_type; }

void Item::set_attack_damage(damage const& dmg) { attack_damage = dmg; }

void Item::set_throw_damage(damage const& dmg) { throw_damage = dmg; }

damage const& Item::get_attack_damage() const { return attack_damage; }

damage const& Item::get_throw_damage() const { return throw_damage; }

int Item::get_hit_plus() const { return hit_plus; }

int Item::get_damage_plus() const { return damage_plus; }

int Item::get_armor() const { return armor; }

void Item::modify_hit_plus(int amount) { hit_plus += amount; }

void Item::modify_damage_plus(int amount) { damage_plus += amount; }

void Item::modify_armor(int amount) { armor += amount; }

void Item::set_hit_plus(int value) { hit_plus = value; }

void Item::set_damage_plus(int value) { damage_plus = value; }

void Item::set_armor(int value) { armor = value; }

void Item::set_cursed() { cursed = true; }

void Item::set_not_cursed() { cursed = false; }

bool Item::is_cursed() const { return cursed; }
...
int Item::probability(Item::Type type) {
  static_assert(25 + 25 + 10 + 10 + 10 + 1 + 4 + 0 + 15 == 100,
                "Item probability");
  switch (type) {
...
static Item::Type random_item_type() {
  ...
  for (int i{0}; i < end; ++i) {
    Item::Type const type{static_cast<Item::Type>(i)};
    int const probability{Item::probability(type)};
...
list<Item*> Item::random_items_chance(int percentage, int dices, int sides) {
  return os_rand_range(100) > percentage ? random_items(dices, sides)
                                         : list<Item*>{};
}
...
bool Item::operator==(Item const& other) const {
  return o_type == other.o_type && o_launch == other.o_launch &&
         o_count == other.o_count && o_which == other.o_which &&
         o_flags == other.o_flags && o_packch == other.o_packch &&

         position_on_screen == other.position_on_screen &&
         nickname == other.nickname && attack_damage == other.attack_damage &&
         throw_damage == other.throw_damage && hit_plus == other.hit_plus &&
         damage_plus == other.damage_plus && armor == other.armor &&
         cursed == other.cursed;
}
