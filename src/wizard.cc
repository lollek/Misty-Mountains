...
static void pr_spec(char type) {
  ...
  switch (type) {
    ...
    default: Game::io->message("Unknown type in pr_spec"); return;
  }
...
int wizard_list_items(void) {
  ...
  vector<char> things{IO::Potion, IO::Scroll, IO::Food, IO::Weapon,
                      IO::Armor,  IO::Ring,   IO::Wand, IO::Trap};
  vector<string> names{
  ...
  switch (type) {
    case IO::Trap: {
      if (which < 0 || which >= Trap::NTRAPS) {
        ...
        Game::level->set_trap_type(player->get_position(),
                                   static_cast<Trap::Type>(which));
      }
    }
      return;
