...
string Monster::get_attack_string(bool successful_hit) const {
  size_t const i{static_cast<size_t>(os_rand_range(4))};

  if (successful_hit) {
    vector<string> hit_string{"hits", "scored an excellent hit on",
                              "has injured", "swings and hits"};
    return hit_string.at(i);

  } else {
    vector<string> miss_string{"misses", "swings and misses", "barely misses",
                               "doesn't hit"};
...
Monster::Monster(int str, int dex, int con, int int_, int wis, int cha, int exp,
                 ...
                 Coordinate const& pos, std::vector<Feat> const& feats,
                 int speed, Race race, char look_, char disguise_,
                 Type subtype_, Coordinate const* target_,
                 std::list<Item*> pack_)
    : Character(str, dex, con, int_, wis, cha, exp, lvl, ac, hp, dmg, pos,
                feats, speed, race),
      look{look_},
      disguise{disguise_},
      subtype{subtype_},
...
Monster::Monster(Monster::Type subtype_, Coordinate const& pos) {
  switch (subtype_) {
    case Bat: {
      *this =
          Monster(3, 15, 11, 2, 14, 4, 100, 1, 14, 4, vector<damage>{{1, 2}},
                  pos, vector<Feat>{}, 2, Race::Animal, 'b', 'b', Monster::Bat,
                  nullptr, list<Item*>{});
    } break;

    case Goblin: {
      *this = Monster(11, 15, 12, 10, 9, 6, 135, 1, 14, 10,
                      vector<damage>{{1, 4}}, pos, vector<Feat>{AttacksOnSight},
                      1, Race::Goblinoid, 'g', 'g', Monster::Goblin, nullptr,
                      Item::random_items_chance(10, 1, 1));
    } break;

    case Kobold: {
      *this =
          Monster(17, 11, 12, 7, 8, 6, 135, 1, 13, 10, vector<damage>{{2, 4}},
                  pos, vector<Feat>{AttacksOnSight}, 1, Race::Orc, 'o', 'o',
                  Monster::Orc, nullptr, Item::random_items_chance(10, 1, 1));
    } break;

    case Orc: {
      *this = Monster(9, 13, 10, 10, 9, 8, 100, 1, 14, 8,
                      vector<damage>{{1, 6}}, pos, vector<Feat>{AttacksOnSight},
                      1, Race::Reptilian, 'k', 'k', Monster::Kobold, nullptr,
                      Item::random_items_chance(10, 1, 1));
    } break;

    case SkeletonHuman: {
      *this = Monster(15, 14, 10, 10, 10, 10, 135, 1, 14, 8,
                      vector<damage>{{1, 6}}, pos, vector<Feat>{AttacksOnSight},
                      1, Race::Undead, 's', 's', Monster::SkeletonHuman,
                      nullptr, Item::random_items_chance(10, 1, 1));
    } break;

    case ZombieHuman: {
      *this =
          Monster(17, 10, 10, 10, 10, 10, 200, 2, 12, 8, vector<damage>{{1, 6}},
                  pos, vector<Feat>{AttacksOnSight}, 1, Race::Undead, 'z', 'z',
                  Monster::ZombieHuman, nullptr, {});
    } break;

    case Hobgoblin: {
      *this = Monster(15, 15, 16, 10, 12, 8, 200, 1, 14, 10,
                      vector<damage>{{1, 8}}, pos, vector<Feat>{AttacksOnSight},
                      1, Race::Goblinoid, 'h', 'h', Monster::Hobgoblin, nullptr,
                      Item::random_items_chance(10, 1, 1));
    } break;

    case Quasit: {
      *this = Monster(8, 14, 11, 11, 12, 11, 600, 3, 14, 10,
                      vector<damage>{{1, 3}}, pos, vector<Feat>{}, 1,
                      Race::Outsider, 'q', 'q', Monster::Quasit, nullptr, {});
    } break;

    case Worg: {
      *this =
          Monster(17, 15, 13, 6, 14, 10, 600, 4, 12, 10, vector<damage>{{1, 6}},
                  pos, vector<Feat>{AttacksOnSight}, 1, Race::MagicBeast, 'w',
                  'w', Monster::Worg, nullptr, {});
    } break;

    case Ogre: {
      *this = Monster(21, 8, 15, 6, 10, 7, 800, 4, 18, 8,
                      vector<damage>{{2, 8}}, pos, vector<Feat>{AttacksOnSight},
                      1, Race::Giant, 'O', 'O', Monster::Ogre, nullptr, {});
    } break;

    case IceMonster: {
      *this =
          Monster(16, 8, 15, 2, 13, 11, 1600, 7, 18, 10, vector<damage>{{1, 8}},
                  pos, vector<Feat>{AttackFreezesTarget}, 1, Race::MagicBeast,
                  'I', 'I', Monster::IceMonster, nullptr, list<Item*>{});
    } break;

    case Yeti: {
      *this = Monster(19, 12, 15, 9, 12, 10, 1200, 6, 16, 10,
                      vector<damage>{{2, 6}}, pos, vector<Feat>{AttacksOnSight},
                      1, Race::MonstrousHumanoid, 'y', 'y', Monster::Yeti,
                      nullptr, list<Item*>{});
    } break;

    case Aquator: {
      *this = Monster(
          10, 17, 13, 2, 13, 8, 800, 5, 15, 8, vector<damage>{{1, 3}}, pos,
          vector<Feat>{AttacksOnSight, AttackRuinsMetal}, 1, Race::Aberration,
          'a', 'a', Monster::Aquator, nullptr, list<Item*>{});
    } break;

    case Troll: {
      *this = Monster(21, 14, 23, 6, 9, 6, 1600, 6, 14, 8,
                      vector<damage>{{1, 8}, {1, 6}}, pos,
                      vector<Feat>{Regenerating5}, 1, Race::Giant, 'T', 'T',
                      Monster::Troll, nullptr, list<Item*>{});
    } break;

    case Erinyes: {
      *this = Monster(20, 23, 21, 14, 18, 21, 4800, 10, 17, 10,
                      vector<damage>{{1, 8}}, pos, vector<Feat>{AttacksOnSight},
                      1, Race::Outsider, 'e', 'e', Monster::Erinyes, nullptr,
                      Item::random_items_chance(20, 1, 1));
    } break;

    case ShadowDemon: {
      *this =
          Monster(10, 18, 17, 14, 14, 19, 3200, 7, 14, 10,
                  vector<damage>{{1, 6}, {1, 6}}, pos,
                  vector<Feat>{AttacksOnSight, PermanentlyInvisible}, 1,
                  Race::Outsider, 'S', 'S', Monster::ShadowDemon, nullptr, {});
    } break;

    case Nightmare: {
      *this = Monster(18, 15, 16, 13, 13, 12, 1600, 6, 17, 10,
                      vector<damage>{{1, 6}, {1, 6}}, pos,
                      vector<Feat>{AttacksOnSight}, 1, Race::Outsider, 'n', 'n',
                      Monster::Nightmare, nullptr, list<Item*>{});
    } break;

    case GenieDjinni: {
      *this = Monster(
          18, 19, 14, 14, 15, 15, 1600, 7, 15, 10, vector<damage>{{1, 8}}, pos,
          vector<Feat>{Planeshift, TurnInvisible}, 1, Race::Outsider, 'g', 'g',
          Monster::GenieDjinni, nullptr, Item::random_items_chance(20, 1, 1));
    } break;

    case GenieVizier: {
      *this = Monster(
          23, 19, 14, 14, 15, 17, 1600, 7, 15, 12, vector<damage>{{1, 8}}, pos,
          vector<Feat>{Planeshift, TurnInvisible}, 1, Race::Outsider, 'g', 'g',
          Monster::GenieVizier, nullptr, Item::random_items_chance(20, 1, 1));
    } break;

    case GenieEfreeti: {
      *this = Monster(23, 17, 18, 12, 14, 15, 4800, 10, 18, 10,
                      vector<damage>{{2, 6}}, pos,
                      vector<Feat>{Planeshift, ScorchingRay, AttacksOnSight}, 1,
                      Race::Outsider, 'g', 'g', Monster::GenieEfreeti, nullptr,
                      Item::random_items_chance(20, 1, 1));
    } break;

    case GenieJanni: {
      *this = Monster(16, 15, 12, 14, 15, 14, 1200, 6, 18, 10,
                      vector<damage>{{1, 6}}, pos, vector<Feat>{}, 1,
                      Race::Outsider, 'g', 'g', Monster::GenieJanni, nullptr,
                      Item::random_items_chance(20, 1, 1));
    } break;

    case GenieMarid: {
      *this = Monster(23, 19, 18, 14, 15, 16, 6400, 12, 19, 10,
                      vector<damage>{{2, 6}, {2, 6}}, pos,
                      vector<Feat>{Planeshift, TurnInvisible}, 1,
                      Race::Outsider, 'g', 'g', Monster::GenieMarid, nullptr,
                      Item::random_items_chance(20, 1, 1));
    } break;

    case GenieShaitan: {
      *this = Monster(20, 13, 19, 14, 14, 15, 3200, 10, 19, 10,
                      vector<damage>{{1, 8}}, pos,
                      vector<Feat>{Planeshift, MoveThroughStone}, 1,
                      Race::Outsider, 'g', 'g', Monster::GenieShaitan, nullptr,
                      Item::random_items_chance(20, 1, 1));
    } break;
    ...
    case AdultRedDragon: {
      *this = Monster(31, 10, 23, 16, 17, 16, 38400, 17, 30, 12,
                      vector<damage>{{2, 6}, {2, 6}}, pos,
                      vector<Feat>{AttacksOnSight, BreatheConeFireMedium}, 1,
                      Race::Dragon, 'D', 'D', Monster::AdultRedDragon, nullptr,
                      Item::random_items(1, 4));
    } break;

    case Jabberwock: {
      *this = Monster(37, 20, 33, 12, 29, 26, 819200, 26, 36, 12,
                      vector<damage>{{3, 6}, {3, 6}}, pos,
                      vector<Feat>{AttacksOnSight}, 1, Race::Dragon, 'J', 'J',
                      Monster::Jabberwock, nullptr, Item::random_items(1, 4));
    } break;
  ...
  if (player != nullptr &&
      player->has_ring_with_ability(Ring::AggravateMonsters)) {
    monster_start_running(&pos);
  }
}

void Monster::set_target(Coordinate const* new_target) { target = new_target; }

Coordinate const* Monster::get_target() const { return target; }

char Monster::get_look() const { return look; }

char Monster::get_disguise() const { return disguise; }
...
void monster_on_death(Monster** monster_ptr, bool print_message) {
...
void monster_remove_from_screen(Monster** monster_ptr, bool was_killed) {
  if (monster_ptr == nullptr) {
    error("monster was null");
  } else if (*monster_ptr == nullptr) {
    error("*monster was null");
...
void Monster::all_move() {
  ...
  while (it != Game::level->monsters.end()) {
    ...
    int const moves{mon->get_moves_this_round()};
    for (int i{0}; i < moves; ++i) {
      bool const wastarget{mon->is_players_target()};
      Coordinate const orig_pos{mon->get_position()};
...
void monster_polymorph(Monster* target) {
  ...
  Monster::Type const monster{Monster::random_monster_type()};
  bool const same_monster{monster == target->get_subtype()};
...
bool monster_try_breathe_fire_on_player(Monster const& monster) {
  ...
  Coordinate const mon_coord{monster.get_position()};
  Coordinate const player_coord{player->get_position()};
  int const dragonshot_chance = 5;
  if (monster.get_subtype() == Monster::AdultRedDragon &&
      (mon_coord.y == player_coord.y || mon_coord.x == player_coord.x ||
       abs(mon_coord.y - player_coord.y) ==
           abs(mon_coord.x - player_coord.x)) &&
...
Monster::Type Monster::get_subtype() const { return subtype; }
...
list<Item*>& Monster::get_pack() { return pack; }
