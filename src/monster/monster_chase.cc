...
static Coordinate chase(Monster& monster, Coordinate const& target) {
  ...
  Coordinate xy;
  for (xy.x = max(mon_pos.x - 1, 0);
       xy.x <= min(mon_pos.x + 1, IO::map_width - 1); xy.x++) {
    for (xy.y = max(mon_pos.y - 1, 0);
         xy.y <= min(mon_pos.y + 1, IO::map_height - 1); xy.y++) {
...
static int chase_do(Monster* monster) {
  ...
  if (chase_coord != monster->get_position()) {
    ...
    if (!Game::level->is_real(chase_coord) &&
        Game::level->get_tile(chase_coord)) {
      was_trap = true;
