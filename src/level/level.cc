...
void Level::create_treasure_room() {
  ...
  int const spots{
      min((room.r_max.y - 2) * (room.r_max.x - 2) - treasure_room_min_items,
          treasure_room_max_items - treasure_room_min_items)};

  int const num_items{os_rand_range(spots) + treasure_room_min_items};
  ...
  int const num_monsters{
      min({os_rand_range(spots) + treasure_room_min_items, num_items + 2,
...
void Level::create_traps() {
  if (os_rand_range(10) < Game::current_level) {
    ...
    for (int i{0}; i < ntraps; ++i) {
      ...
      set_not_real(coord);
      Trap::Type const trap_type{
          static_cast<Trap::Type>(os_rand_range(Trap::NTRAPS))};
...
Tile& Level::tile(int x, int y) {
  size_t const pos{static_cast<size_t>((x << 5) + y)};
  return tiles.at(pos);
}

Monster* Level::get_monster(int x, int y) { return tile(x, y).monster; }
...
bool Level::is_passage(int x, int y) { return tile(x, y).is_passage; }

bool Level::is_passage(Coordinate const& coord) {
  return is_passage(coord.x, coord.y);
}

bool Level::is_discovered(int x, int y) { return tile(x, y).is_discovered; }

bool Level::is_discovered(Coordinate const& coord) {
  return is_discovered(coord.x, coord.y);
}

bool Level::is_real(int x, int y) { return tile(x, y).is_real; }

bool Level::is_real(Coordinate const& coord) {
  return is_real(coord.x, coord.y);
}

void Level::set_passage(int x, int y) { tile(x, y).is_passage = true; }

void Level::set_passage(Coordinate const& coord) {
  set_passage(coord.x, coord.y);
}

void Level::set_discovered(int x, int y) { tile(x, y).is_discovered = true; }

void Level::set_discovered(Coordinate const& coord) {
  set_discovered(coord.x, coord.y);
}

void Level::set_real(int x, int y) { tile(x, y).is_real = true; }

void Level::set_real(Coordinate const& coord) { set_real(coord.x, coord.y); }

void Level::set_not_real(int x, int y) { tile(x, y).is_real = false; }

void Level::set_not_real(Coordinate const& coord) {
  set_not_real(coord.x, coord.y);
}

Tile::Type Level::get_tile(int x, int y) { return tile(x, y).type; }

Tile::Type Level::get_tile(Coordinate const& coord) {
  return get_tile(coord.x, coord.y);
}

void Level::set_tile(int x, int y, Tile::Type type) { tile(x, y).type = type; }
...
Trap::Type Level::get_trap_type(int x, int y) { return tile(x, y).trap_type; }
...
bool Level::can_step(int x, int y) {
  switch (get_tile(x, y)) {
    ...
    case Tile::Shop: return false;

    case Tile::Floor:
    case Tile::OpenDoor:
    case Tile::StairsDown:
    case Tile::Trap:
    case Tile::StairsUp: break;
  }
...
bool Level::is_dark(int x, int y) { return tile(x, y).is_dark; }
