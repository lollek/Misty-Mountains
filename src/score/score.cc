...
static bool lock_sc(void) {
  ...
  if (time(nullptr) - sbuf.st_mtime > 10)
    return unlink(LOCKFILE) < 0 ? false : lock_sc();

  printf(
      "The score file is very busy.  Do you want to wait longer\n"
      "for it to become free so your score can get posted?\n"
      "If so, type \"y\"\n");

  return Game::io->readchar(true) == 'y' ? lock_sc() : false;
}
...
static void score_read(struct score* top_ten) {
  ...
  for (unsigned i{0}; i < SCORE_MAX; i++) {
    ...
    sscanf(buf, " %u %d %d %d %d %x \n", &top_ten[i].uid, &top_ten[i].score,
           &top_ten[i].flags, &top_ten[i].death_type, &top_ten[i].level,
           &top_ten[i].time);
  }
...
static void score_insert(struct score* top_ten, int amount, int death_type) {
  unsigned uid{getuid()};
  for (unsigned i{0}; i < SCORE_MAX; ++i)
...
static void score_print(struct score* top_ten) {
  Game::io->stop_curses();
  printf("Top %d %s:\n   Score Name\n", SCORE_MAX, "Scores");
  for (unsigned i{0}; i < SCORE_MAX; ++i) {
...
void score_show_and_exit(int death_type) {
  int amount{0};
  ...
  if (!wizard) { score_insert(top_ten, amount, death_type); }
